...
#include <math.h>
//**************中斷使用須知*********************
//必須要先進行連結(只須連結一次)  MO_InterruptCase(0, NULL, mo.MoInterrupt);//中斷連結
/***********************************************************
**                                                        **
**          運動模組-運動指令命令 (對應動作判斷)             **
**                                                        **
...
CAction::CAction()
{
    ...
    m_TablelZ = 60850;//工作平台高度(Z軸總工作高度)
	m_MachineCirMid.x = 0;//機械同心圓座標x
	m_MachineCirMid.y = 0;//機械同心圓座標y
	m_MachineOffSet.x = 0;//機械同心圓偏移量x
	m_MachineOffSet.y = 0;//機械同心圓偏移量y
	m_Wangle = 0;//機械校正角度w
	...
	LONG lAcceleration, LONG lInitVelociy)
{
	...
	*/
    ...
    {
      lX = W_GetNeedlePoint(lX, lY, dAng, 1).x;
      lY = W_GetNeedlePoint(lX, lY, dAng, 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
	...
	LONG lLowVelocity, int iType, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
	...
	*/
	...
	GelatinizeBack(iType, W_GetNeedlePoint(lX, lY, dAng, 1).x, W_GetNeedlePoint(lX, lY, dAng, 1).y, lZ, cpNeedlePoint.x, cpNeedlePoint.y, lDistance, lHigh, lZBackDistance, lLowVelocity, lHighVelocity, lAcceleration, lInitVelociy);//返回設定
	...
	LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
	...
	*/
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        lX = W_GetNeedlePoint(lX, lY, dAng, 1).x;
        lY = W_GetNeedlePoint(lX, lY, dAng, 1).y;
        lX2 = W_GetNeedlePoint(lX, lY, dAng2, 1).x;
        lY2 = W_GetNeedlePoint(lX, lY, dAng2, 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
...
void CAction::DecideCircle(LONG lX1, LONG lY1, LONG lZ1, DOUBLE dAng1, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAng2, LONG lWorkVelociy, LONG lInitVelociy)
{
	...
	*/
    ...
    if (USEMODE_W == 1)//使用機械座標
    {     
        lX1 = W_GetNeedlePoint(lX1, lY1, dAng1, 1).x;
        lY1 = W_GetNeedlePoint(lX1, lY1, dAng1, 1).y;
        lX2 = W_GetNeedlePoint(lX2, lY2, dAng2, 1).x;
        lY2 = W_GetNeedlePoint(lX2, lY2, dAng2, 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
    ...
    if (!m_bIsStop)
    {
        MO_DO4Curve(pDataShift, num-1, lWorkVelociy);
...
void CAction::DecideArc(LONG lX1, LONG lY1, LONG lZ1, DOUBLE dAng1, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAng2, LONG lWorkVelociy, LONG lInitVelociy)
{
	...
	*/
    ...
    DOUBLE SumAng = dAng2 - MO_ReadLogicPositionW();
    size_t num = VecBuf.size();
    DOUBLE num_Ang = SumAng / (DOUBLE)num;
    DATA_4MOVE *pDataM = new DATA_4MOVE[num];
    DATA_4MOVE *pDataShift = pDataM;
    for (size_t i = 0; i<VecBuf.size() - 1; i++)
    {
...
void CAction::DecideLineSToCirP(LONG lX1, LONG lY1, LONG lZ1, DOUBLE dAng1, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAng2, LONG lX3, LONG lY3, LONG lZ3, DOUBLE dAng3, LONG lStartDelayTime, LONG lStartDistance, LONG lMidDelayTime, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
    ...
    */
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        ...
        lX2 = W_GetNeedlePoint(lX2, lY2, dAng2, 1).x;
        lY2 = W_GetNeedlePoint(lX2, lY2, dAng2, 1).y;
        lX3 = W_GetNeedlePoint(lX2, lY2, dAng3, 1).x;
        lY3 = W_GetNeedlePoint(lX2, lY2, dAng3, 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
...
void CAction::DecideCirclePToEnd(LONG lX1, LONG lY1, LONG lZ1, DOUBLE dAng1, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAng2, LONG lX3, LONG lY3, LONG lZ3, DOUBLE dAng3, LONG lCloseOffDelayTime, LONG lCloseDistance, LONG lCloseONDelayTime, LONG lZBackDistance, BOOL bZDisType, LONG lHighVelocity, LONG lDistance, LONG lHigh, LONG lLowVelocity, int iType, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
	...
	*/
	...
	if ((W_GetNeedlePoint().x >= lX3 - 5) && (W_GetNeedlePoint().x <= lX3 + 5) && (W_GetNeedlePoint().y >= lY3 - 5) && (W_GetNeedlePoint().y <= lY3 + 5)) //表示結束點在起始點上
	{
        ...
        if (!m_bIsStop)
        {
            MO_DO4Curve(pDataShift, num - 1, lWorkVelociy);//執行四軸插補
            PreventMoveError();//防止驅動錯誤
        }
        pDataShift = NULL;
        delete[] pDataM;
...
void CAction::DecideArclePToEnd(LONG lX1, LONG lY1, LONG lZ1, DOUBLE dAng1, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAng2, LONG lCloseOffDelayTime, LONG lCloseDistance, LONG lCloseONDelayTime, LONG lZBackDistance, BOOL bZDisType, LONG lHighVelocity, LONG lDistance, LONG lHigh, LONG lLowVelocity, int iType, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
	...
	*/
    ...
    {
        //求三維圓心與半徑
        Circle.ArcCentCalculation_2D(dp1, dp2, dp3, dRad, angle);
        pCen = Circle.SpaceCircleCenterCalculation(dp1, dp2, dp3, dRad, angle);
        Circle.ArcCutPoint(p1, p2, p3, VecBuf); //3d切圓
    }
    DOUBLE SumAng = dAng2 - MO_ReadLogicPositionW();
    size_t num = VecBuf.size();
    DOUBLE num_Ang = 360 / (DOUBLE)num;
    ...
    if (lCloseDistance != 0) //線段點膠設定---(5)關機距離
    {
        dSumPath = M_PI * dRad * (angle /360.0);//移動的總長度
        if (lCloseDistance != 0 && lCloseDistance > (LONG)round(dSumPath))
        {
            lCloseDistance = (LONG)round(dSumPath);
        }
...
void CAction::DecideParkPoint(LONG lX, LONG lY, LONG lZ, DOUBLE dAng, LONG lTimeGlue, LONG lWaitTime, LONG lStayTime, LONG lZBackDistance, BOOL bZDisType, LONG lZdistance, LONG lHighVelocity, LONG lLowVelocity, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
	...
	*/
    ...
    lX = W_GetMachinePoint(lX, lY, dAng, 1).x;//針頭座標轉換成機械座標
    lY = W_GetMachinePoint(lX, lY, dAng, 1).y;//針頭座標轉換成機械座標
...
void CAction::DecideInitializationMachine(LONG lSpeed1, LONG lSpeed2, LONG lAxis, LONG lMoveX, LONG lMoveY , LONG lMoveZ, DOUBLE dMoveW)
{
    ...
    */
    ...
    if (lAxis > 7)
    {
        DecideGoHomeW(lSpeed1, lSpeed2);//*******************先檢察mcc************2017/1/12!!!!!**************************
        lAxis = lAxis - 8;
    }
    MO_SetHardLim(lAxis, 1);
    MO_SetSoftLim(lAxis, 0);	//TODO::軟體極限要記得改
    if(!m_bIsStop)
    {
...
void CAction::DecideGoHomeW(LONG lSpeed1, LONG lSpeed2)
{
	...
	*/
	...
	if (!m_bIsStop)
	{
		MO_MoveToHomeW(lSpeed1, lSpeed2, 8, 0, 0, 0, bBuf);
		PreventMoveError();//防止軸卡出錯
	}
...
void CAction::DecideVirtualHome(LONG lX, LONG lY, LONG lZ, LONG lZBackDistance, BOOL bZDisType, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        lX = W_GetNeedlePoint(lX, lY, 0, 1).x;
        lY = W_GetNeedlePoint(lX, lY, 0, 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
    DecideGoHomeW(lWorkVelociy, 2000);//w軸復歸
    lX = W_GetMachinePoint(lX, lY, 0, 1).x;//針頭座標轉換成機械座標
    lY = W_GetMachinePoint(lX, lY, 0, 1).y;//針頭座標轉換成機械座標
	if (lZBackDistance <= 0)
	{
		return;
	}
	...
	LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
	...
	*/
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        ...
        lX1 = W_GetNeedlePoint(lX1, lY1, MO_ReadLogicPositionW(), 1).x;
        lY1 = W_GetNeedlePoint(lX1, lY1, MO_ReadLogicPositionW(), 1).y;
        lX2 = W_GetNeedlePoint(lX2, lY2, MO_ReadLogicPositionW(), 1).x;
        lY2 = W_GetNeedlePoint(lX2, lY2, MO_ReadLogicPositionW(), 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
    lX1 = W_GetMachinePoint(lX1, lY1, MO_ReadLogicPositionW(), 1).x;
    lY1 = W_GetMachinePoint(lX1, lY1, MO_ReadLogicPositionW(), 1).y;
    lX2 = W_GetMachinePoint(lX2, lY2, MO_ReadLogicPositionW(), 1).x;
    lY2 = W_GetMachinePoint(lX2, lY2, MO_ReadLogicPositionW(), 1).y;
...
CString CAction::NowOffSet(LONG lX, LONG lY, LONG lZ,DOUBLE dAng)
{
    ...
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
    lX = W_GetMachinePoint(lX, lY, dAng, 1).x;
    lY = W_GetMachinePoint(lX, lY, dAng, 1).y;
...
void CAction::DoCCDMove(LONG lX, LONG lY, LONG lZ, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
	...
	*/
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        lX = W_GetNeedlePoint(lX, lY, MO_ReadLogicPositionW(), 1).x;
        lY = W_GetNeedlePoint(lX, lY, MO_ReadLogicPositionW(), 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
    lX = W_GetMachinePoint(lX, lY, MO_ReadLogicPositionW(), 1).x;
    lY = W_GetMachinePoint(lX, lY, MO_ReadLogicPositionW(), 1).y;
	if (!m_bIsStop)
	{
		MO_Do3DLineMove(lX - MO_ReadLogicPosition(0), lY - MO_ReadLogicPosition(1), 0, lWorkVelociy, lAcceleration, lInitVelociy);//x,y軸移動
		PreventMoveError();//防止軸卡出錯
...
//----------------------------------------------------
//軟體負極限(x,y,z預設為-10)
void CAction::HMNegLim(LONG lX, LONG lY, LONG lZ, DOUBLE lW)
{
#ifdef MOVE
	MO_SetSoftLim(15, 1);
	MO_SetCompSoft(1, -lX, -lY, -lZ,-lW);
#endif
}
//軟體正極限(x,y,z)
void CAction::HMPosLim(LONG lX, LONG lY, LONG lZ,DOUBLE lW)
{
#ifdef MOVE
	MO_SetSoftLim(15, 1);
	MO_SetCompSoft(0, lX, lY, lZ,lW);
#endif
}
/***********************************************************
**                                                        **
...
void CAction::LA_Do2DVetor(LONG lX3, LONG lY3, LONG lX2, LONG lY2, LONG lX1, LONG lY1)
{
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        cpbuf[0].x = W_GetNeedlePoint(lX1, lY1, MO_ReadLogicPositionW(), 1).x;
        cpbuf[0].y = W_GetNeedlePoint(lX1, lY1, MO_ReadLogicPositionW(), 1).y;
        cpbuf[1].x = W_GetNeedlePoint(lX2, lY2, MO_ReadLogicPositionW(), 1).x;
        cpbuf[1].y = W_GetNeedlePoint(lX2, lY2, MO_ReadLogicPositionW(), 1).y;
        lX3 = W_GetNeedlePoint(lX3, lY3, MO_ReadLogicPositionW(), 1).x;
        lY3 = W_GetNeedlePoint(lX3, lY3, MO_ReadLogicPositionW(), 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
	BOOL bType = 0;
	...
	else
	{
        ...
        DATA_2D.CirCentP.x = W_GetMachinePoint(machine[0], machine[1], MO_ReadLogicPositionW(), 1).x;
        DATA_2D.CirCentP.y = W_GetMachinePoint(machine[0], machine[1], MO_ReadLogicPositionW(), 1).y;
		DATA_2D.CirRev = CStringToLong(csbuff, 2);//取得圓心(X，Y，Rev，)
	}
	DATA_2D.EndP.x = W_GetMachinePoint(lX3, lY3, MO_ReadLogicPositionW(), 1).x;
	DATA_2D.EndP.y = W_GetMachinePoint(lX3, lY3, MO_ReadLogicPositionW(), 1).y;
	DATA_2D.Type = bType;
	DATA_2D.Speed = 0;
...
void CAction::LA_Do2dDataLine(LONG EndPX, LONG EndPY, BOOL bIsNeedleP)
{
    ...
    if (USEMODE_W == 1 && bIsNeedleP == 0)//使用機械座標
    {
        EndPX = W_GetNeedlePoint(EndPX, EndPY, MO_ReadLogicPositionW(), 1).x;
        EndPY = W_GetNeedlePoint(EndPX, EndPY, MO_ReadLogicPositionW(), 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
	DATA_2MOVE DATA_2D;
	DATA_2D.EndP.x = W_GetMachinePoint(EndPX, EndPY, MO_ReadLogicPositionW(), 1).x;
	DATA_2D.EndP.y = W_GetMachinePoint(EndPX, EndPY, MO_ReadLogicPositionW(), 1).y;
	DATA_2D.Type = 0;//直線
	DATA_2D.Speed = 0;
	DATA_2D.CirCentP.x = 0;
	DATA_2D.CirCentP.y = 0;
...
void CAction::LA_Do2dDataArc(LONG EndPX, LONG EndPY, LONG ArcX, LONG ArcY)
{
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        EndPX = W_GetNeedlePoint(EndPX, EndPY, MO_ReadLogicPositionW(), 1).x;
        EndPY = W_GetNeedlePoint(EndPX, EndPY, MO_ReadLogicPositionW(), 1).y;
        ArcX = W_GetNeedlePoint(ArcX, ArcY, MO_ReadLogicPositionW(), 1).x;
        ArcY = W_GetNeedlePoint(ArcX, ArcY, MO_ReadLogicPositionW(), 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
	DATA_2MOVE DATA_2D;
...
void CAction::LA_Do2dDataCircle(LONG EndPX, LONG EndPY, LONG CirP1X, LONG CirP1Y, LONG CirP2X, LONG CirP2Y)
{
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        EndPX = W_GetNeedlePoint(EndPX, EndPY, MO_ReadLogicPositionW(), 1).x;
        EndPY = W_GetNeedlePoint(EndPX, EndPY, MO_ReadLogicPositionW(), 1).y;
        CirP1X = W_GetNeedlePoint(CirP1X, CirP1Y, MO_ReadLogicPositionW(), 1).x;
        CirP1Y = W_GetNeedlePoint(CirP1X, CirP1Y, MO_ReadLogicPositionW(), 1).y;
        CirP2X = W_GetNeedlePoint(CirP2X, CirP2Y, MO_ReadLogicPositionW(), 1).x;
        CirP2Y = W_GetNeedlePoint(CirP2X, CirP2Y, MO_ReadLogicPositionW(), 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
	DATA_2MOVE DATA_2D;
...
void CAction::LA_AverageZ(LONG lStrX, LONG lStrY, LONG lEndX, LONG lEndY, LONG &lZ, LONG lStartVe, LONG lStartAcc, LONG lStartInitVe, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
    ...
    if (USEMODE_W == 1)//使用機械座標
    {
        lStrX = W_GetNeedlePoint(lStrX, lStrY, MO_ReadLogicPositionW(), 1).x;
        lStrY = W_GetNeedlePoint(lStrX, lStrY, MO_ReadLogicPositionW(), 1).y;
        lEndX = W_GetNeedlePoint(lEndX, lEndY, MO_ReadLogicPositionW(), 1).x;
        lEndY = W_GetNeedlePoint(lEndX, lEndY, MO_ReadLogicPositionW(), 1).y;
    }
    /*******以下程式都會轉變使用針頭座標執行運動************************/
    //流程: 輸入針頭座標>運算針頭座標>轉換成機械座標輸出
	m_LaserAverage = TRUE;
...
void CAction::W_UpdateNeedleMotorOffset(CPoint &offset, DOUBLE degree)
{
	...
	DOUBLE degreeNow = MO_ReadLogicPositionW();
	offset.x = (LONG)round(tmpOffset.x*cos((degree - degreeNow)*M_PI / 180.0) - tmpOffset.y*sin((degree - degreeNow)*M_PI / 180.0));
	offset.y = (LONG)round(tmpOffset.x*sin((degree - degreeNow)*M_PI / 180.0) + tmpOffset.y*cos((degree - degreeNow)*M_PI / 180.0));
#endif
}
...
void CAction::W_UpdateNeedleMotor_Robot(LONG lX, LONG lY, LONG lZ, DOUBLE dAngle0, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAngle1, DOUBLE DisAngle)
{
  ...
  //  LONG Distance = (LONG)sqrt(pow(NeedlePointStart.x - NeedlePointEnd.x, 2) + pow(NeedlePointStart.y - NeedlePointEnd.y, 2));//線段總長度
    int idx = (int)round((dAngle1 - dAngle0) / DisAngle);//固定角度走幾個點
    LONG idx_xVec = (LONG)round((NeedlePointEnd.x- NeedlePointStart.x) / (DOUBLE)idx), idx_yVec = (LONG)round((NeedlePointEnd.y - NeedlePointStart.y) / (DOUBLE)idx), idx_zVec = (LONG)round((lZ2-lZ) / (DOUBLE)idx);//每個點走多遠
    if (idx > 511)
    {
    ...
    for (int i = 1; i < idx; i++)
    {
        W_UpdateNeedleMotorOffset(cpoffsetbuf, dAngle0 + DisAngle * i);//OFFSET與角度轉換
        ...
        Data4buf.AngleW = dAngle0 + DisAngle * i;//w絕對角度
...
void CAction::W_UpdateNeedleMotor_Needle(LONG lX, LONG lY, LONG lZ, DOUBLE dAngle0, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAngle1, DOUBLE DisAngle)
{
	...
	int idx = (int)round((dAngle1 - dAngle0) / DisAngle);//固定角度走幾個點
	LONG idx_xVec = (LONG)round((lX2-lX) / (DOUBLE)idx), idx_yVec = (LONG)round((lY2-lY) / (DOUBLE)idx), idx_zVec = (LONG)round((lZ2-lZ) / (DOUBLE)idx);//每個點走多遠
	//LONG idx_dis = (LONG)round((DOUBLE)Distance / (DOUBLE)idx);//每個點走多遠
	DATA_4MOVE Data4buf = { 0 };//傳值用
	if (idx > 511)
	{
		AfxMessageBox(L"切值角度過小請重新設定");
	...
	{
		W_UpdateNeedleMotorOffset(cpoffsetbuf, dAngle0 + DisAngle * i);//OFFSET與角度轉換
		//LineGetToPoint(DATA_4Do[i].EndPX, DATA_4Do[i].EndPY, lX2, lY2, DATA_4Do[i - 1].EndPX, DATA_4Do[i - 1].EndPY, idx_dis);
...
void CAction::W_AbsToOppo4Move(std::vector<DATA_4MOVE> &str)
{
    ...
    DATA_4MOVE mData;
    CPoint cpNeedlePointNow = W_GetNeedlePoint();
    std::vector<DATA_4MOVE> vecBuf;
    vecBuf.clear();
    mData.EndPX = str.at(0).EndPX - cpNeedlePointNow.x;
    mData.EndPY = str.at(0).EndPY - cpNeedlePointNow.y;
    mData.EndPZ = str.at(0).EndPZ - MO_ReadLogicPosition(2);
    str.at(0).EndPZ = mData.EndPZ;
...
void CAction::W_Line4DtoDo(LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy)
{
    ...
    if (!m_bIsStop)
    {
        MO_Do4DLineMove(W_m_ptVec.at(0).EndPX - cpNeedlePoint.x, W_m_ptVec.at(0).EndPY- cpNeedlePoint.y,0, W_m_ptVec.at(0).AngleW - MO_ReadLogicPositionW(), lWorkVelociy, lAcceleration, lInitVelociy);//w旋轉(x,y,w同動)
        PreventMoveError();//防止軸卡出錯
    }
...
void CAction::W_Correction(BOOL bStep, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy,LONG lMoveZ)
{
	...
	if (!bStep)
	{
		...
		MO_Do3DLineMove(0, 0, lMoveZ - MO_ReadLogicPosition(2), 50000, 150000, 2000);//z軸抬升到10000位置
		PreventMoveError();
		MO_Do4DLineMove(0, 0, 0, 180, 20000, 50000, 2000);//W軸旋轉180度
		PreventMoveError();
	...
	else
	{
		...
		m_MachineOffSet.x = m_MachineCirMid.x - MO_ReadLogicPosition(0);//取得offsetx
		m_MachineOffSet.y = m_MachineCirMid.y - MO_ReadLogicPosition(1);//取得offsety
...
void CAction::MCO_JogMove(LONG lX, LONG lY, LONG lZ, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy, DOUBLE dW, BOOL WType)
{
	...
	switch (sw)
	{
        ...
        case 2:
        {
            if (WType == 0)//W單軸移動
            {
                MO_Do4DLineMove(0, 0, 0, dW, lWorkVelociy, lAcceleration, lInitVelociy);//w軸單軸移動
            }
            else //W自轉 已針頭為中心旋轉
            {
