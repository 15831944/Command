...
CAction::CAction()
{
    ...
    m_HomingPoint.x = 200000;   //原點復歸點X(機械座標)
    m_HomingPoint.y = 200000;   //原點復歸點Y(機械座標)
...
void CAction::DecideCircle(LONG lX1, LONG lY1, LONG lZ1, DOUBLE dAng1, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAng2, LONG lWorkVelociy, LONG lInitVelociy)
{
    ...
    */
    ...
    for(size_t i = 0; i<VecBuf.size()-1; i++)
    {
        if(i == 0)
        {
            cpMpbuf = W_GetMachinePoint(VecBuf.at(i).x, VecBuf.at(i).y, MO_ReadLogicPositionW(), 1);
        ...
        {
            cpMpbuf = W_GetMachinePoint(VecBuf.at(i).x, VecBuf.at(i).y, MO_ReadLogicPositionW() + (num_Ang * i), 1);
            pDataShift->EndPX = cpMpbuf.x - VecBuf.at(i - 1).x;
            pDataShift->EndPY = cpMpbuf.y - VecBuf.at(i - 1).y;
...
void CAction::DecideInitializationMachine(LONG lSpeed1, LONG lSpeed2, LONG lAxis, LONG lMoveX, LONG lMoveY, LONG lMoveZ, DOUBLE dMoveW)
{
    ...
    */
    ...
    {
        MO_MoveToHome(lSpeed1, lSpeed2, 4, 0, 0, lMoveZ);
        PreventMoveError();//防止軸卡出錯
        MO_SetSoftLim(4, 1);
        lAxis = lAxis - 4;
    }
    ...
    {
        DecideGoHomeW(lSpeed1, lSpeed2);//*******************先檢察mcc************2017/3/8!!!!!**************************
        lAxis = lAxis - 8;
    }
...
void CAction::DecideGoHomeW(LONG lSpeed1, LONG lSpeed2)
{
    ...
    */
    ...
    if(MO_ReadLogicPositionW() < 0.0)
    {
        bBuf = 1;//順時針轉
        //下列動作為逆時針轉((負逆
        if(MO_ReadLogicPositionW() <= -360.0 || HMbuf[4] == 1)
        {
            MO_AlarmCClean();//解除錯誤
    ...
    else
    {
        bBuf = 0;//逆時針轉
        //下列動作為順時針轉((正順
        if(MO_ReadLogicPositionW() >= 360.0 || HMbuf[3] == 1)
        {
            MO_AlarmCClean();//解除錯誤
            MO_Do4DLineMove(0, 0, 0, - 90, lSpeed1, LONG(lSpeed1*m_WSpeed), lSpeed2);
            PreventMoveError();
    ...
    if(!m_bIsStop)
    {
        MO_MoveToHomeW(lSpeed1, lSpeed2, 8, 0, 0, 0, 0, bBuf);
        PreventMoveError();//防止軸卡出錯
    }
    ...
    else if(((CAction *)param)->m_ThreadFlag == 2)
    {
        ...
        else
        {
            ...
            ((CAction *)param)->DecideInitializationMachine(((CAction *)param)->cpCirMidBuff[0].x, ((CAction *)param)->cpCirMidBuff[0].y, 15,
                    (((CAction *)param)->m_HomingPoint.x + ((CAction *)param)->m_MachineOffSet.x), (((CAction *)param)->m_HomingPoint.y + ((CAction *)param)->m_MachineOffSet.y), ((CAction *)param)->m_HomingPoint.z, ((CAction *)param)->m_HomingPoint.w);
            if(!((CAction *)param)->m_bIsStop)
            {
...
void CAction::W_UpdateNeedleMotor_Robot(LONG lX, LONG lY, LONG lZ, DOUBLE dAngle0, LONG lX2, LONG lY2, LONG lZ2, DOUBLE dAngle1, DOUBLE DisAngle)
{
    ...
    {
        W_UpdateNeedleMotorOffset(cpoffsetbuf, dAngle0 + DisAngle * i*((idx>0) ? 1 : -1));//OFFSET與角度轉換
        ...
        Data4buf.AngleW = dAngle0 + DisAngle * i*((idx>0) ? 1 : -1);//w絕對角度
...
void CAction::W_Rotation(DOUBLE dAngle, LONG lWorkVelociy, LONG lAcceleration, LONG lInitVelociy, DOUBLE DisAngle)
{
    ...
    DATA_4MOVE data4M = { 0 };
    W_m_ptVec.clear();
    for(int i = 0; i <= (LONG)abs(dAngle / DisAngle); i++)
    {
        W_UpdateNeedleMotorOffset(offset, MO_ReadLogicPositionW() + DisAngle*i*((dAngle>0) ? 1 : -1));//依角度更新offset
        //針頭座標轉回機械座標(絕對座標)
        data4M.EndPX = cpNeedlePoint[0].x - offset.x;
        data4M.EndPY = cpNeedlePoint[0].y - offset.y;
        data4M.EndPZ = MO_ReadLogicPosition(2);
        data4M.AngleW = MO_ReadLogicPositionW() + DisAngle*i*((dAngle>0) ? 1 : -1);
