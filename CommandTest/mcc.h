...
void MO_AbsToOppo3Move(std::vector<DATA_3MOVE> &);//絕對位置轉成相對位置-3軸插補
...
MO_DLL void MO_MoveToHome(LONG lSpeed1, LONG lSpeed2, int iAxis, LONG lMoveX,
                          LONG lMoveY, LONG lMoveZ);//原點復歸-多載(速度1，速度2，指定軸7，偏移量X,Y,Z)
...
MO_DLL void MO_Do3DLineMove(LONG lXTar, LONG lYTar, LONG lZTar, LONG lSpeed, LONG lAcceleration,
                            LONG lInitSpeed);//三軸直線插補
...
MO_DLL void MO_ReadRunHardLim(int &iPMLimX, BOOL &bIsOkLimX, int &iPMLimY, BOOL &bIsOkLimY, int &iPMLimZ,
...
MO_DLL void MO_GummingSet(int iData,
                          LONG lMicroSecond);//設定出膠時間-多載(選擇多個點膠埠1~3,出膠時間(usec),中斷程序名稱)
...
MO_DLL void MO_ReadEvent(long *RR1X, long *RR1Y, long *RR1Z, long *RR1U);//讀取中斷暫存器RR1
MO_DLL CString MO_StreamToUnit(int iValue);//數字轉換成CString 16-bit binary
MO_DLL void MO_Do2ArcReduce(LONG lXTar, LONG lYTar, LONG XCenter, LONG YCenter, LONG lInitSpeed, LONG lSpeed,
                            BOOL bRevolve, LONG lAcceleration, LONG  lPation,
                            LPTHREAD_START_ROUTINE StopGumming = NULL);//XY兩軸圓弧補間移動(減速點)
MO_DLL void MO_Do3DLineMoveInterr(LONG lXTar, LONG lYTar, LONG lZTar,
                                  LONG lSpeed, LONG lAcceleration, LONG lInitSpeed, LPTHREAD_START_ROUTINE
                                  LPInterrupt = NULL); //XYZ三軸直線補間移動 - 多載(中斷)
...
MO_DLL void MO_DllDate(CString &dllDate);//DLL 更新時間
